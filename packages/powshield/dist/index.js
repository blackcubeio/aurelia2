...
const IPowshieldService = aurelia.DI.createInterface('IPowshieldService', (x) => x.singleton(PowshieldService));
class PowshieldService {
    constructor(logger = aurelia.resolve(aurelia.ILogger), httpService = aurelia.resolve(IHttpService), options = aurelia.resolve(IPowshieldConfiguration), platform = aurelia.resolve(aurelia.IPlatform)) {
        this.logger = logger;
...
let Powshield = (() => {
    let _classDecorators = [aurelia.customAttribute("bc-powshield")];
    ...
    _classThis = class {
        constructor(element = aurelia.resolve(aurelia.INode), logger = aurelia.resolve(aurelia.ILogger).scopeTo('Powshield'), options = aurelia.resolve(IPowshieldConfiguration), powshieldService = aurelia.resolve(IPowshieldService), platform = aurelia.resolve(aurelia.IPlatform)) {
    ...
    (() => {
        ...
        _solutionInputSelector_decorators = [aurelia.bindable({ primary: true })];
        _solutionReadyChanged_decorators = [aurelia.watch('solutionReady')];
...
exports.Powshield = Powshield;
exports.PowshieldConfiguration = PowshieldConfiguration;
exports.PowshieldService = PowshieldService;
